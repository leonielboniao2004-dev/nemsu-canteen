...
export const AppShell = ({ children, title, subtitle, }) => {
    ...
    return (<div className="min-h-screen" style={{ background: "var(--gradient-page)" }}>
      <header className="bg-card/75 backdrop-blur-xl border-b border-border/60 sticky top-0 z-30 shadow-[0_1px_0_0_hsl(var(--border)/0.4)]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <Link to="/dashboard" className="flex items-center gap-3 shrink-0 group">
            <div className="h-10 w-10 rounded-xl flex items-center justify-center transition-transform duration-300 group-hover:scale-105 group-hover:rotate-3" style={{ background: "var(--gradient-primary)", boxShadow: "var(--shadow-button)" }}>
          ...
          <nav className="flex items-center gap-1 overflow-x-auto scrollbar-none">
            ...
            return (<Link key={to} to={to} className={`relative flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${active
                    ? "text-primary-foreground shadow-[var(--shadow-button)]"
                    : "text-muted-foreground hover:text-foreground hover:bg-secondary/70"}`} style={active ? { background: "var(--gradient-primary)" } : undefined}>
          ...
          <div className="flex items-center gap-2">
            <NotificationsBell />
            {!isAdmin && (<Link to="/cart" className="relative">
                <Button variant="outline" size="icon" className="relative hover:bg-secondary hover:text-primary transition-colors">
                  <ShoppingCart className="h-4 w-4"/>
                  {count > 0 && (<Badge className="absolute -top-2 -right-2 h-5 min-w-5 px-1 rounded-full bg-primary text-primary-foreground border-2 border-card animate-fade-in">
                      {count}
                    </Badge>)}
                </Button>
              </Link>)}
