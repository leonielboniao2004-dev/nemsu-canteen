...
import { Pencil, Camera, Lock, Bell, Shield, ChevronRight, CreditCard, Plus, Wallet, CalendarCheck, CheckCircle2, Clock, ArrowRight, HelpCircle, MoreVertical, Upload, BadgeCheck, FileImage, X, ShieldCheck, Loader2, } from "lucide-react";
import { StudentSidebarLayout } from "@/components/StudentSidebarLayout";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { getSession, updateSession, uploadAvatar, uploadIdCard } from "@/lib/auth";
...
const Settings = () => {
    ...
    return (<StudentSidebarLayout title="Settings" subtitle="Manage your personal information and account settings.">
      <div className="grid xl:grid-cols-[1fr_340px] gap-6">
        ...
        <div className="space-y-6">
          ...
          <section className="bg-card border border-border rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              {editing ? (<div className="flex gap-2">
                ...
                </div>) : (<Button variant="gradient" size="sm" className="gap-2" onClick={() => setEditing(true)}>
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex flex-col items-center gap-3 shrink-0">
                <div className="relative">
                  {session.avatar ? (<img src={session.avatar} alt={session.name} className="h-28 w-28 rounded-full object-cover border-4 border-card shadow-sm"/>) : (<div className="h-28 w-28 rounded-full flex items-center justify-center text-2xl font-bold text-primary-foreground" style={{ background: "var(--gradient-primary)" }}>
                      {initials}
                    </div>)}
                  <button onClick={() => avatarInput.current?.click()} disabled={uploadingAvatar} className="absolute bottom-1 right-1 h-9 w-9 rounded-full bg-primary text-primary-foreground flex items-center justify-center border-2 border-card hover:opacity-90 transition-opacity disabled:opacity-50" aria-label="Change photo">
                    {uploadingAvatar ? <Loader2 className="h-4 w-4 animate-spin"/> : <Camera className="h-4 w-4"/>}
                  </button>
                </div>
                <input ref={avatarInput} type="file" accept="image/*" className="hidden" onChange={(e) => onAvatarPick(e.target.files?.[0] ?? null)}/>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => avatarInput.current?.click()} disabled={uploadingAvatar}>
                    {uploadingAvatar ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <Upload className="h-3.5 w-3.5"/>} Upload
                  </Button>
                  {session.avatar && (<Button variant="ghost" size="sm" onClick={removeAvatar}>
          ...
          <section className="bg-card border border-border rounded-2xl p-6">
            ...
            <ul className="divide-y divide-border border border-border rounded-xl">
              {/* Verification row */}
              <li>
                <VerificationRow idCardImage={session.idCardImage} submitting={submittingId} onPick={() => idInput.current?.click()}/>
                <input ref={idInput} type="file" accept="image/*" className="hidden" onChange={(e) => onIdPick(e.target.files?.[0] ?? null)}/>
              </li>
              {settingsRows.map(({ icon: Icon, title, desc, onClick }) => (<li key={title}>
                  <button onClick={onClick} className="w-full flex items-center gap-4 p-4 hover:bg-muted/30 transition-colors text-left">
          ...
          <section className="bg-card border border-border rounded-2xl p-6">
            ...
            <button onClick={() => toast.info("Add payment method coming soon")} className="mt-3 w-full border border-dashed border-border rounded-xl py-3.5 text-sm font-medium text-primary hover:bg-primary/5 transition-colors inline-flex items-center justify-center gap-2">
        ...
        <aside className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            ...
            const t = tints[tint];
            return (<li key={title} className="flex items-start gap-3">
...
const VerificationRow = ({ idCardImage, submitting, onPick }) => {
    const status = idCardImage ? "verified" : "unverified";
    return (<div className="p-4 flex flex-col sm:flex-row sm:items-center gap-4">
      <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${status === "verified"
            ? "bg-[hsl(var(--success))]/15 text-[hsl(var(--success))]"
            : "bg-primary/10 text-primary"}`}>
        {status === "verified" ? <BadgeCheck className="h-4 w-4"/> : <ShieldCheck className="h-4 w-4"/>}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="text-sm font-semibold">Account Verification</p>
          {status === "verified" && (<Badge className="bg-[hsl(var(--success))]/15 text-[hsl(var(--success))] hover:bg-[hsl(var(--success))]/15 border-0">Verified</Badge>)}
          {status === "unverified" && (<Badge variant="outline">Not verified</Badge>)}
        </div>
        ...
        {idCardImage && (<div className="mt-3 inline-flex items-center gap-2 text-xs text-muted-foreground border border-border rounded-lg px-3 py-1.5">
            <FileImage className="h-3.5 w-3.5"/>
            ID Card uploaded
          </div>)}
      </div>
      <div className="flex gap-2 shrink-0">
        <Button size="sm" variant={status === "verified" ? "outline" : "gradient"} onClick={onPick} disabled={submitting}>
            {submitting ? <><Loader2 className="h-3.5 w-3.5 animate-spin"/> Submitting</> : <><Upload className="h-3.5 w-3.5"/> {status === "verified" ? "Update ID" : "Upload ID"}</>}
        </Button>
      </div>
    </div>);
};
const Field = ({ label, value, editing, onChange }) => (<div>
    <p className="text-xs text-muted-foreground font-medium mb-1">{label}</p>
    {editing ? (<input value={value} onChange={(e) => onChange?.(e.target.value)} className="w-full text-sm font-semibold bg-transparent border-b border-border focus:border-primary outline-none py-1"/>) : (<p className="text-sm font-semibold">{value}</p>)}
  </div>);
