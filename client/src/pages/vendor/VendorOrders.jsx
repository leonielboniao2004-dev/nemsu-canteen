...
const VendorOrders = () => {
    ...
    const visible = useMemo(() => {
        ...
        if (q.trim()) {
            ...
            list = list.filter((o) => ((o.shortId || o._id || o.id) + o.studentName + o.studentEmail + o.studentId).toLowerCase().includes(s));
        }
    ...
    const updateStatus = async (o, status, label) => {
        try {
            await updateOrderStatus(o._id || o.id, status);
            toast.success(`${o.shortId || o._id} → ${label}`);
            refetch();
        } catch {
    ...
    return (<VendorShell title="Order Queue" subtitle="Accept, prepare, and complete student orders.">
      ...
      {visible.length === 0 ? (<div className="bg-card rounded-2xl p-12 text-center border border-border">
          ...
          {visible.map((o) => (<div key={o._id || o.id} className="bg-card rounded-2xl p-5 border border-border">
              <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-bold">{o.shortId || o._id}</p>
