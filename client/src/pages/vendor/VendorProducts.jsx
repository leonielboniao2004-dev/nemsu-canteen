...
const VendorProducts = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      list = list.filter((p) =>
        `${p.name ?? ""} ${p.description ?? ""} ${p.sku ?? ""}`.toLowerCase().includes(term)
      );
    }
    return list;
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.error(editing ? "Product update failed" : "Product add failed", {
  ...
  const removeProduct = async (p) => {
    ...
    try {
      ...
      toast.error("Product remove failed", { description: getErrorMessage(err) });
    }
  };

  const toggleProduct = async (p) => {
    try {
      await productsStore.toggleAvailable(p.id);
      await refetch();
    } catch (err) {
      toast.error("Product update failed", { description: getErrorMessage(err) });
    }
  };
  ...
  const doRestock = async () => {
    ...
    try {
      await productsStore.restock(restock.p.id, qty);
      await refetch();
      toast.success(`Added ${qty} to ${restock.p.name}`);
      setRestock(null);
    } catch (err) {
      toast.error("Restock failed", { description: getErrorMessage(err) });
    }
  };
                    ...
                    <tr key={p.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1 max-w-[260px]">
                              {p.description}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="px-5 py-4">
                        {!p.available ? (
                          ...
                          <Badge className="bg-[hsl(var(--warning))]/15 text-[hsl(var(--warning))] border-0 rounded-md">
                            Low Stock
                          </Badge>
                        ) : (
                          <Badge className="bg-[hsl(var(--success))]/15 text-[hsl(var(--success))] border-0 rounded-md">
                            Available
                          </Badge>
                        )}
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <Button
                            ...
                            aria-label={p.available ? "Hide" : "Show"}
                          >
                            {p.available ? (
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="py-2">
            <p className="text-sm text-muted-foreground mb-3">
              Current stock: <span className="font-semibold text-foreground">{restock?.p.stock}</span>
            </p>
            ...
            <Input
              ...
              min={1}
              value={restock?.qty ?? ""}
              onChange={(e) => restock && setRestock({ ...restock, qty: e.target.value })}
              className="h-11"
            />
          </div>
          <DialogFooter>
