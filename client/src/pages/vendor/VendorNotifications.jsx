...
import { ICONS, notificationsStore, TYPE_ICON_BG, timeAgo, useNotifications } from "@/lib/notifications";
import { toast } from "sonner";
const TABS = ["All", "Unread", "Pending", "Alerts", "Announcements"];
const VendorNotifications = () => {
    ...
    const visible = useMemo(() => {
        ...
        if (tab === "Pending")
            return items.filter((n) => n.type === "Pending");
        if (tab === "Alerts")
      ...
      <div className="bg-card border border-border rounded-2xl">
        <div className="p-5 flex flex-wrap items-center justify-between gap-3 border-b border-border">
          <div className="flex flex-wrap gap-2">
            {TABS.map((t) => (<button key={t} onClick={() => setTab(t)} className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${tab === t ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-secondary"}`}>
                {t}
              </button>))}
          </div>
        ...
        <ul className="divide-y divide-border">
          {visible.length === 0 ? (<li className="text-center py-12 text-muted-foreground text-sm">No notifications.</li>) : visible.map((n) => {
            ...
            return (<li key={n.id} className="px-5 py-4 flex items-start gap-3 hover:bg-muted/30 transition-colors">
                ...
                <Badge className={`${TYPE_ICON_BG[n.type]} border-0 rounded-md font-medium self-center`}>{n.type}</Badge>
                {!n.read && (<button onClick={() => notificationsStore.markRead(n.id)} className="h-8 px-2 text-xs rounded-lg text-muted-foreground hover:bg-secondary self-center">
                    Read
                  </button>)}
              </li>);
        })}
