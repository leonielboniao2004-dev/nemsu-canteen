...
import { cartStore, cartTotal, useEnrichedCart, useCart } from "@/lib/cart";
...
const Cart = () => {
    const cart = useCart();
    const { enriched: detailed, loading: cartLoading } = useEnrichedCart();
    ...
    const placeOrder = async () => {
        ...
        if (detailed.length === 0 || cartLoading) return;
        setPlacing(true);
        try {
            ...
            cartStore.clear();
            toast.success("Order placed!", { description: `Reference ${order.shortId} · pickup at ${order.pickupTime}` });
            navigate("/orders");
        } catch (err) {
    ...
    return (<AppShell title="Your Cart" subtitle="Review and confirm your pre-order">
      ...
      {detailed.length === 0 ? (<div className="bg-card rounded-2xl p-12 text-center" style={{ boxShadow: "var(--shadow-card)" }}>
          ...
          <div className="lg:col-span-2 bg-card rounded-2xl p-2 sm:p-4" style={{ boxShadow: "var(--shadow-card)" }}>
            <ul className="divide-y divide-border">
              {detailed.map((line) => (<li key={line.id} className="flex items-center gap-3 sm:gap-4 p-3">
            ...
            <div className="mt-5 space-y-3">
              <div>
                ...
                <div className="grid grid-cols-3 gap-1.5 mt-1.5">
                  {PICKUPS.map((p) => (<button key={p} type="button" onClick={() => setPickup(p)} className={`text-xs py-2 rounded-lg border transition-colors ${pickup === p ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border hover:bg-secondary"}`}>
                      {p}
                    </button>))}
                </div>
              </div>
