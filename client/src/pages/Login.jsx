...
import { loginStudent, loginVendor } from "@/lib/auth";
...
const Index = () => {
    ...
    const handleStudentLogin = async (e) => {
        ...
        try {
            await loginStudent({ email, password, role: portalRole });
            toast.success("Welcome back!", { description: `Logged in as ${portalRole}.` });
            navigate("/dashboard");
        } catch (err) {
    ...
    const handleAdminLogin = async (e) => {
        e.preventDefault();
        const errs = {};
        if (!adminKey.trim()) errs.adminKey = "Vendor key is required.";
        setErrors(errs);
        if (Object.keys(errs).length) return;

        setAdminLoading(true);
        try {
            await loginVendor({ vendorKey: adminKey.trim(), password: adminPassword });
            toast.success("Welcome, Vendor!");
            navigate("/dashboard");
        } catch (err) {
            console.error("Vendor login error:", err);
            const msg = err.response?.data?.error || "Access denied.";
            setErrors({ adminKey: msg });
    ...
    return (<AuthLayout illustration={illustration} illustrationAlt={alt}>
      <Tabs value={tab} onValueChange={(v) => setTab(v)} className="w-full">
        ...
        <div className={tab === "student" ? "mb-5 grid grid-cols-2 gap-2" : "hidden"}>
          <button type="button" onClick={() => setPortalRole("student")} className={`h-11 rounded-xl text-sm font-semibold border transition-colors ${portalRole === "student" ? "bg-primary/10 border-primary text-primary" : "border-border text-muted-foreground hover:bg-secondary"}`}>
            🎓 Student
          </button>
          <button type="button" onClick={() => setPortalRole("teacher")} className={`h-11 rounded-xl text-sm font-semibold border transition-colors ${portalRole === "teacher" ? "bg-primary/10 border-primary text-primary" : "border-border text-muted-foreground hover:bg-secondary"}`}>
            🧑‍🏫 Teacher
          </button>
        </div>
