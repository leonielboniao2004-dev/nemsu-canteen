...
import { Bell, Mail, CheckCircle2, Megaphone, AlertTriangle, CheckCheck, History, Settings, ChevronRight, ChevronLeft, ChevronRight as ChevronR, } from "lucide-react";
import { StudentSidebarLayout } from "@/components/StudentSidebarLayout";
import { Badge } from "@/components/ui/badge";
import { Switch } from "@/components/ui/switch";
import { toast } from "sonner";
import { getSession } from "@/lib/auth";
import { ICONS, notificationsStore, timeAgo, TYPE_ICON_BG, useNotifications, } from "@/lib/notifications";
const TYPE_PILL = TYPE_ICON_BG;
const TABS = ["All", "Unread", "Announcements", "Alerts", "System"];
const PAGE_SIZE = 8;
const Notifications = () => {
    ...
    const filtered = useMemo(() => {
        ...
        if (tab === "Alerts")
            return items.filter((n) => n.type === "Alert");
      ...
      <div className="grid xl:grid-cols-[1fr_320px] gap-6">
        <div className="bg-card border border-border rounded-2xl">
          ...
          <div className="px-5 pt-4 flex flex-wrap gap-2">
            {TABS.map((t) => (<button key={t} onClick={() => { setTab(t); setPage(1); }} className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${tab === t ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-secondary"}`}>
                {t}
              </button>))}
          </div>

          <ul className="divide-y divide-border mt-2">
            {visible.length === 0 ? (<li className="text-center py-12 text-muted-foreground text-sm">No notifications in this view.</li>) : (visible.map((n) => {
            const Icon = ICONS[n.iconName] ?? ICONS.Mail;
            return (<li key={n.id} className="px-5 py-4 flex items-start gap-3 hover:bg-muted/30 transition-colors">
                    ...
                    <button onClick={() => markRead(n.id)} className="h-8 px-2 rounded-lg text-xs text-muted-foreground hover:bg-secondary self-center">
                      Read
                    </button>
                  </li>);
        }))}
          ...
          <div className="p-5 flex flex-wrap items-center justify-between gap-3 border-t border-border">
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: totalPages }).map((_, i) => (<button key={i} onClick={() => setPage(i + 1)} className={`h-8 min-w-8 px-2 rounded-lg border text-sm ${page === i + 1 ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-secondary"}`}>
                  {i + 1}
                </button>))}
        ...
        <aside className="space-y-4">
          <div className="bg-card border border-border rounded-2xl p-5">
            ...
            return (<li key={s.label} className="flex items-center justify-between">
...
const QuickAction = ({ icon: Icon, title, desc, onClick }) => (<li>
...
const PrefRow = ({ icon: Icon, title, desc, defaultOn }) => {
...
const NStat = ({ icon, label, value, tint }) => {
    const tints = {
