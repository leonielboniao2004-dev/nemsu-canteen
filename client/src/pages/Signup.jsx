...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (role === "student") {
      if (!/^\d{6,12}$/.test(form.studentId))
        errs.studentId = "Student ID must be 6-12 digits.";
    }
    else {
      if (form.studentId.trim().length < 3)
        errs.studentId = "Employee ID is required.";
    }
    if (!/^[\d+\-\s]{7,15}$/.test(form.phone))
      errs.phone = "Enter a valid phone number.";
    if (!form.grade)
      errs.grade = role === "teacher" ? "Select your department." : "Select your grade.";
    if (role === "student" && !form.section)
      errs.section = "Select your section.";
    ...
    try {
      ...
      toast.success("Account created!", { description: "Welcome to the canteen system!" });
      navigate("/dashboard");
    } catch (err) {
  ...
  return (<AuthLayout illustration={studentImg} illustrationAlt="Student signup illustration">
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        {["student", "teacher"].map((r) => (<button key={r} type="button" onClick={() => setRole(r)} className={`h-11 rounded-xl text-sm font-semibold border transition-colors ${role === r
...
const Field = ({ label, icon, error, children }) => (<div className="space-y-1.5">
