...
export const AuthLayout = ({ illustration, illustrationAlt, children, subtitle = "Student & Admin Portal" }) => {
    return (<div className="min-h-screen w-full flex flex-col relative overflow-hidden" style={{ background: "var(--gradient-page)" }}>
      {/* Decorative mesh background */}
      <div aria-hidden className="pointer-events-none absolute inset-0" style={{ background: "var(--gradient-mesh)" }}/>
      {/* Soft floating orbs */}
      ...
      <main className="relative flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-5xl bg-card/95 backdrop-blur-xl rounded-3xl overflow-hidden grid md:grid-cols-2 border border-border/50 animate-slide-up" style={{ boxShadow: "var(--shadow-elevated)" }}>
          {/* Illustration side */}
          <div className="hidden md:flex flex-col items-center justify-center p-10 relative overflow-hidden" style={{ background: "var(--gradient-soft)" }}>
            ...
            <img src={illustration} alt={illustrationAlt} loading="lazy" width={1024} height={1024} className="relative max-h-[420px] w-auto object-contain drop-shadow-xl"/>
          </div>
