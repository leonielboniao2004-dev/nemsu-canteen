...
export const NotificationsBell = () => {
  ...
  const markOne = (id: string) => notificationsStore.markRead(id);

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          className="relative h-10 w-10 rounded-xl bg-secondary flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors"
          aria-label={`Notifications${unread ? `, ${unread} unread` : ""}`}
        >
          <Bell className="h-4 w-4" />
          {unread > 0 && (
            <span className="absolute -top-1 -right-1 h-5 min-w-5 px-1 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold flex items-center justify-center border-2 border-card">
              {unread}
            </span>
          )}
        </button>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          ...
          <button
            onClick={markAll}
            className="text-xs font-medium text-primary inline-flex items-center gap-1 hover:underline disabled:opacity-50 disabled:no-underline"
            disabled={unread === 0}
          >
        ...
        <div className="border-t border-border p-2 flex items-center justify-between">
          <Link
            to={audience === "vendor" ? "/vendor/notifications" : "/notifications"}
            onClick={() => setOpen(false)}
            className="text-xs font-medium text-primary px-3 py-2 hover:underline"
          >
            View all notifications
          </Link>
          <Link
            to={audience === "vendor" ? "/vendor/settings" : "/settings"}
            onClick={() => setOpen(false)}
            className="text-xs font-medium text-muted-foreground hover:text-foreground inline-flex items-center gap-1 px-3 py-2"
          >
