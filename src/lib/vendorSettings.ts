...
export const readVendorSettings = (): VendorSettings => {
  if (typeof window === "undefined") return defaultVendorSettings;
  try {
  ...
  } catch { return defaultVendorSettings; }
};

export const writeVendorSettings = (s: VendorSettings) => {
...
export const useVendorSettings = () => {
  const [s, setS] = useState<VendorSettings>(readVendorSettings);
  useEffect(() => {
