...
const NEXT_ACTION: Partial<Record<OrderStatus, { to: OrderStatus; label: string; icon: React.ElementType }>> = {
...
const AdminOrders = () => {
  ...
  const updateStatus = (o: Order, status: OrderStatus) => {
  ...
  return (
    <AppShell title="Order Queue" subtitle="Manage incoming canteen orders from your vendor portal">
      ...
      {visible.length === 0 ? (
        ...
        <div className="space-y-4">
          {visible.map((o) => {
            ...
            return (
              <div key={o.id} className="bg-card rounded-2xl p-5" style={{ boxShadow: "var(--shadow-card)" }}>
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
                  {next && (
                    <Button size="sm" onClick={() => updateStatus(o, next.to)} className="gap-1.5">
                      <next.icon className="h-4 w-4" /> {next.label}
                    </Button>
                  )}
                  {o.status !== "Cancelled" && o.status !== "Completed" && (
                    <Button size="sm" variant="outline" className="gap-1.5 text-destructive hover:text-destructive" onClick={() => updateStatus(o, "Cancelled")}>
