...
const Settings = () => {
    ...
    const verification = session.verification ?? { status: "unverified" };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex flex-col items-center gap-3 shrink-0">
                <div className="relative">
                  {session.avatarDataUrl ? (<img src={session.avatarDataUrl} alt={session.name} className="h-28 w-28 rounded-full object-cover border-4 border-card shadow-sm"/>) : (<div className="h-28 w-28 rounded-full flex items-center justify-center text-2xl font-bold text-primary-foreground" style={{ background: "var(--gradient-primary)" }}>
                      {initials}
                    </div>)}
                  <button onClick={() => avatarInput.current?.click()} className="absolute bottom-1 right-1 h-9 w-9 rounded-full bg-primary text-primary-foreground flex items-center justify-center border-2 border-card hover:opacity-90 transition-opacity" aria-label="Change photo">
                ...
                <div className="flex gap-2">
                  ...
                  {session.avatarDataUrl && (<Button variant="ghost" size="sm" onClick={removeAvatar}>
          ...
          <section className="bg-card border border-border rounded-2xl p-6">
            ...
            <ul className="divide-y divide-border border border-border rounded-xl">
              {/* Verification row */}
              <li>
                <VerificationRow verification={verification} submitting={submittingId} onPick={() => idInput.current?.click()} onCancel={cancelVerification}/>
                <input ref={idInput} type="file" accept="image/*,application/pdf" className="hidden" onChange={(e) => onIdPick(e.target.files?.[0] ?? null)}/>
              </li>
...
const VerificationRow = ({ verification, submitting, onPick, onCancel, }) => {
    ...
    return (<div className="p-4 flex flex-col sm:flex-row sm:items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {status === "pending" && (<Badge className="bg-[hsl(var(--warning))]/15 text-[hsl(var(--warning))] hover:bg-[hsl(var(--warning))]/15 border-0">Pending review</Badge>)}
        ...
        {verification.idDataUrl && (<div className="mt-3 inline-flex items-center gap-2 text-xs text-muted-foreground border border-border rounded-lg px-3 py-1.5">
            <FileImage className="h-3.5 w-3.5"/>
            ID file on record
          </div>)}
      </div>
      <div className="flex gap-2 shrink-0">
        {status !== "verified" && (<Button size="sm" variant="gradient" onClick={onPick} disabled={submitting}>
        ...
        {status !== "unverified" && (<Button size="sm" variant="outline" onClick={onCancel}>Reset</Button>)}
      </div>
    </div>);
};
