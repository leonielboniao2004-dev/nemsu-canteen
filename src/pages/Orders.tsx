...
import { useOrders, ordersStore, OrderStatus, Order } from "@/lib/orders";
import { peso } from "@/lib/menu";
import { toast } from "sonner";

type DisplayStatus = "Pending" | "Accepted" | "Completed" | "Cancelled";

const toDisplay = (s: OrderStatus): DisplayStatus =>
  s === "Preparing" || s === "Ready" ? "Accepted" : (s as DisplayStatus);
...
const Orders = () => {
  ...
  const mine = useMemo(() => {
    if (!session || session.role !== "student" && session.role !== "teacher") return [];
    return all
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter !== "All") list = list.filter((o) => toDisplay(o.status) === statusFilter);
    return list;
  ...
  const summary: { label: DisplayStatus; count: number }[] = (["Pending", "Accepted", "Completed", "Cancelled"] as DisplayStatus[]).map((s) => ({
    label: s,
    count: mine.filter((o) => toDisplay(o.status) === s).length,
  }));

  const cancel = (o: Order) => {
    ordersStore.setStatus(o.id, "Cancelled");
    toast.success(`${o.id} cancelled`);
  };

  if (!session || session.role !== "student" && session.role !== "teacher") return null;

  return (
    <StudentSidebarLayout title="My Reservations" subtitle="View and manage all your canteen reservations.">
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={8} className="text-center py-12 text-muted-foreground">No reservations to show.</td></tr>
                  ) : (
                    filtered.map((o, idx) => {
                      ...
                      const sku = `#RES-${String(23 - idx).padStart(5, "0")}`;
                      return (
                        <tr key={o.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                          ...
                          <td className="px-5 py-4">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              {ds === "Pending" && (
                                <button onClick={() => cancel(o)} className="h-8 px-3 rounded-lg border border-destructive/30 text-destructive text-xs font-medium hover:bg-destructive/10">Cancel</button>
                              )}
                            </div>
                          </td>
                        </tr>
...
const addMonth = (d: Date, delta: number) => {
  const n = new Date(d);
  n.setMonth(n.getMonth() + delta);
  return n;
};

const CalendarCard = ({ month, onPrev, onNext, orders }: { month: Date; onPrev: () => void; onNext: () => void; orders: Order[] }) => {
...
const STAT_TINTS: Record<string, { bg: string; fg: string; link: string }> = {
...
const StatCard = ({ icon, label, value, tint, link }: { icon: React.ReactNode; label: string; value: string; tint: "primary" | "success" | "warning" | "info"; link: string }) => {
