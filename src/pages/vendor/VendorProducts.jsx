...
const empty = { name: "", description: "", price: "", category: "Meals", emoji: "🍽️", stock: "10", available: true };
const VendorProducts = () => {
    ...
    const filtered = useMemo(() => {
        ...
        if (status === "Low")
            list = list.filter((p) => productsStore.isLow(p) && p.stock > 0);
        if (q.trim())
            list = list.filter((p) => (p.name + p.description + p.sku).toLowerCase().includes(q.toLowerCase()));
        return list;
    ...
    return (<VendorShell title="Product Management" subtitle="Add, edit, restock, and toggle availability of menu items." toolbar={<Button onClick={openAdd} className="gap-2"><Plus className="h-4 w-4"/> Add Product</Button>}>
      {/* Filters */}
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (<tr><td colSpan={7} className="text-center py-12 text-muted-foreground">No products match your filters.</td></tr>) : filtered.map((p) => {
            const out = p.stock <= 0;
            const low = productsStore.isLow(p) && !out;
            return (<tr key={p.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                    ...
                    <td className="px-5 py-4">
                      {!p.available ? (<Badge className="bg-muted text-muted-foreground border-0 rounded-md">Hidden</Badge>) : out ? (<Badge className="bg-destructive/10 text-destructive border-0 rounded-md">Out</Badge>) : low ? (<Badge className="bg-[hsl(var(--warning))]/15 text-[hsl(var(--warning))] border-0 rounded-md">Low Stock</Badge>) : (<Badge className="bg-[hsl(var(--success))]/15 text-[hsl(var(--success))] border-0 rounded-md">Available</Badge>)}
                    </td>
                    <td className="px-5 py-4">
