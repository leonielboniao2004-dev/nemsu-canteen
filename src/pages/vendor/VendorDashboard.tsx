...
import {
  Package, ShoppingBag, AlertTriangle, Users, ChefHat, CheckCircle2,
  ArrowRight, BarChart3, Plus, ClipboardList, Boxes, FileText,
} from "lucide-react";
...
const VendorDashboard = () => {
  ...
  return (
    <VendorShell title="Vendor Dashboard" subtitle="Live overview of your canteen operations">
...
}: { icon: React.ReactNode; label: string; value: string; tint: keyof typeof TINTS; link: { to: string; label: string } }) => {
  const t = TINTS[tint];
  return (
    <div className="bg-card border border-border rounded-2xl p-5">
...
const QuickAction = ({
  to, icon: Icon, label, desc, tint,
}: { to: string; icon: React.ElementType; label: string; desc: string; tint: keyof typeof TINTS }) => {
  const t = TINTS[tint];
  return (
    <Link to={to} className="rounded-xl border border-border p-4 hover:border-primary hover:bg-primary/5 transition-colors text-center">
