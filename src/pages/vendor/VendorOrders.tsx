...
import {
  ChefHat, PackageCheck, CheckCircle2, XCircle, Clock, ShieldCheck, Inbox, Search, Check,
} from "lucide-react";
import { VendorShell } from "@/components/VendorShell";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { ordersStore, useOrders, OrderStatus, Order } from "@/lib/orders";
...
const VendorOrders = () => {
  ...
  const [filter, setFilter] = useState<(typeof TABS)[number]["key"]>("Pending");
  ...
  const counts = useMemo(() => {
    ...
    for (const t of TABS) if (t.key !== "All") c[t.key] = orders.filter((o) => o.status === t.key).length;
    return c;
  }, [orders]);

  const updateStatus = (o: Order, status: OrderStatus, label: string) => {
  ...
  return (
    <VendorShell title="Order Queue" subtitle="Accept, prepare, and complete student orders.">
          ...
          {visible.map((o) => (
            <div key={o.id} className="bg-card rounded-2xl p-5 border border-border">
              ...
              <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
                ...
                {o.status === "Preparing" && (
                  <Button size="sm" onClick={() => updateStatus(o, "Ready", "Ready")} className="gap-1.5">
                    <PackageCheck className="h-4 w-4" /> Mark ready
                  </Button>
                )}
                {o.status === "Ready" && (
                  <Button size="sm" onClick={() => updateStatus(o, "Completed", "Completed")} className="gap-1.5">
                    <CheckCircle2 className="h-4 w-4" /> Mark served
                  </Button>
                )}
                {(o.status === "Preparing" || o.status === "Ready") && (
                  <Button size="sm" variant="outline" className="gap-1.5 text-destructive hover:text-destructive" onClick={() => updateStatus(o, "Cancelled", "Cancelled")}>
                    <XCircle className="h-4 w-4" /> Cancel
                  </Button>
                )}
                {(o.status === "Completed" || o.status === "Cancelled") && (
                  <p className="text-xs text-muted-foreground self-center">Updated {new Date(o.updatedAt).toLocaleString()}</p>
                )}
              </div>
            </div>
          ))}
...
const Stat = ({ icon, label, value, tint }: { icon: React.ReactNode; label: string; value: string; tint: "primary" | "success" | "warning" }) => {
