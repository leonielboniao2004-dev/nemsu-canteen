...
const VendorCustomers = () => {
  ...
  return (
    <VendorShell title="Customers" subtitle="View customer profiles and order history.">
        ...
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-12 text-muted-foreground">No customers yet.</td></tr>
                ) : filtered.map((c) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b border-border last:border-0 cursor-pointer transition-colors ${isActive ? "bg-accent/40" : "hover:bg-muted/30"}`}
                    >
              ...
              <div className="mt-5 pt-5 border-t border-border">
                ...
                {active.recent.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {active.recent.map((o) => (
                      <li key={o.id} className="flex items-center justify-between text-sm border border-border rounded-lg px-3 py-2">
                        <div>
...
const SmallStat = ({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) => (
...
const KV = ({ k, v, cap, className = "" }: { k: string; v: string; cap?: boolean; className?: string }) => (
