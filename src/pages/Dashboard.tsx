...
const Dashboard = () => {
  ...
  if (session.role === "admin") return <VendorDashboard />;
...
const StudentDashboard = () => {
  ...
  const [status, setStatus] = useState<"All" | "Available" | "Low Stock">("All");
  ...
  const filtered = useMemo(() => {
    ...
    if (status !== "All") list = list.filter((m) => (status === "Low Stock" ? m.stock < 6 : m.stock >= 6));
    if (q.trim()) list = list.filter((m) => (m.name + " " + m.description).toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <StudentSidebarLayout
      title="Products"
      subtitle="Manage and organize all canteen products."
      toolbar={
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard
          ...
          link={{ to: "/menu", label: "View all products" }}
        />
        <StatCard
          ...
          link={{ to: "/menu", label: "View available" }}
        />
        <StatCard
          ...
          link={{ to: "/menu", label: "View low stock" }}
        />
        <StatCard
          ...
          link={{ to: "/menu", label: "Manage categories" }}
        />
      </div>
          ...
          <div className="p-5 flex flex-wrap items-center justify-between gap-3 border-t border-border">
            ...
            <div className="flex items-center gap-1">
              {["‹", "1", "2", "3", "›"].map((p, i) => (
                <button
                  key={i}
                  className={`h-8 min-w-8 px-2 rounded-lg border text-sm ${p === "1" ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-secondary"}`}
                >
                  {p}
                </button>
              ))}
...
const ProductRow = ({ p }: { p: MenuItem & { sku: string; stock: number } }) => {
  const low = p.stock < 6;
  return (
    <tr className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
      ...
      <td className="px-5 py-4">
        {low ? (
          <Badge className="bg-[hsl(var(--warning))]/15 text-[hsl(var(--warning))] hover:bg-[hsl(var(--warning))]/15 border-0 rounded-md">Low Stock</Badge>
        ) : (
          <Badge className="bg-[hsl(var(--success))]/15 text-[hsl(var(--success))] hover:bg-[hsl(var(--success))]/15 border-0 rounded-md">Available</Badge>
        )}
      </td>
      <td className="px-5 py-4">
        <div className="flex items-center justify-end gap-1.5">
          <button
            disabled={low}
            onClick={() => { cartStore.add(p.id, 1); toast.success("Added to cart", { description: p.name }); }}
            className="h-8 px-3 rounded-lg bg-primary text-primary-foreground hover:opacity-90 flex items-center gap-1.5 text-xs font-medium disabled:opacity-40 disabled:cursor-not-allowed"
          >
...
const STAT_TINTS: Record<string, { bg: string; fg: string }> = {
...
const STAT_LINK_COLORS: Record<string, string> = {
...
const StatCard = ({
  icon, label, value, tint, link,
}: { icon: React.ReactNode; label: string; value: string; tint: "primary" | "success" | "warning" | "info"; link: { to: string; label: string } }) => {
...
const AdminDashboard = () => {
  ...
  const stocked = useMemo(
    () => MENU.map((m, i) => ({ ...m, sku: `PRD-${String(32 - i).padStart(5, "0")}`, stock: ((i * 7) % 18) + 2 })),
    []
  );
  ...
  return (
    <AppShell title="Vendor Dashboard" subtitle="Live overview of your canteen operations">
      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
...
const StatusRow = ({ label, value, dot }: { label: string; value: number; dot: string }) => (
