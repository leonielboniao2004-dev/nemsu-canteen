    ...
    return (<AuthLayout illustration={illustration} illustrationAlt={alt}>
      <Tabs value={tab} onValueChange={(v) => setTab(v)} className="w-full">
        ...
        <div className={tab === "student" ? "mb-5 grid grid-cols-2 gap-2" : "hidden"}>
          <button type="button" onClick={() => setPortalRole("student")} className={`h-11 rounded-xl text-sm font-semibold border transition-colors ${portalRole === "student"
            ? "bg-primary/10 border-primary text-primary"
            : "border-border text-muted-foreground hover:bg-secondary"}`}>
            🎓 Student
          </button>
          <button type="button" onClick={() => setPortalRole("teacher")} className={`h-11 rounded-xl text-sm font-semibold border transition-colors ${portalRole === "teacher"
            ? "bg-primary/10 border-primary text-primary"
            : "border-border text-muted-foreground hover:bg-secondary"}`}>
            🧑‍🏫 Teacher
          </button>
